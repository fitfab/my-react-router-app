...
export const CarouselViewport = ({ width, height, ...rest }: CarouselProps): React.ReactElement => {
  const navigationHeight = "48px";
  const dynamicStyles = {
    width,
    height: `calc(${height} + ${navigationHeight})`,
  };
  return <div style={dynamicStyles} className={`relative mx-[auto] flex flex-wrap`} {...rest} />;
};

export const Steering = (props: React.HTMLAttributes<HTMLDivElement>): React.ReactElement => {
  return <div className="flex flex-wrap justify-end content-center w-full pt-2" {...props} />;
};
